...
间距、字号层级，以及 reset / `runs out` 的文案，都取自 Paseo 自己的用量组件，因此面板读起来与设置页接近。
两处不同：服务商品牌图标来自宿主内部的图标注册表，插件无法引用，所以每行以服务商名称开头；进度条则用
插件自己的色阶，而不是宿主的 status token——见 [配色](#配色)。
...
- **颜色是量出来的，不是猜的。** 主题色只能通过 surface 的 props 传给插件，而这个仪表是 React 之外的 DOM
  ...
  轨道色与次要前景色 token 来画外围。进度条填充色则来自插件自己的色阶（见 [配色](#配色)），探测结果只
  决定用其中的明色版还是暗色版。遇到无法识别的主题（比如插件贡献的主题）则回退到该行实际渲染的文字颜色，
  并按亮度在明/暗两套配色中选一套。颜色每两秒重新探测一次，所以切换主题无需重载即可生效。探测时会跳过与行
...
进度条按 蓝 → 橙 → 红 三级递进。两个界面共用同一张表（`shared/usage/palette.ts`）——此前它们是分开的：
仪表里写死了一套颜色字面量，而面板大部分填充色来自宿主的 `theme.colors.status*` token，改任何一边都会
悄无声息地和另一边对不上。
...
| **红** | 超过 90%，或守护进程预测会在重置前耗尽的窗口 | 要么现在处理，要么等重置。 |
| **灰** | 没有可用的百分比 | 不是「用得少」，是「读不到」。 |

绿色是刻意不用的。绿色读作「良好」，等于把眼睛唯一的强信号花在了根本不需要注意的状态上；蓝色才是中性的
「这里没你的事」，这样暖色就只表达一件事，橙→红也就成了整块区域里唯一一次颜色跳变。

每条进度条最终的色调，取 **服务商上报的色调** 与 **百分比推导出的色调** 中更严重的那个。完全听服务商的，
等于让它的阈值决定本插件的色阶在哪里跳变，而且不上报色调的服务商会一路画成灰色；完全本地推导又会丢掉
百分比表达不了的信息——一个被预测提前耗尽的窗口，在 40% 就该是红的。

填充色不使用宿主的 status token。那些 token 是为文字调的，浅色背景要求深墨：Light 主题下 Paseo 的
`statusWarning` 是一个深琥珀，当 4px 色块用时读起来是棕色，`statusSuccess` 则接近黑色。橙色是这套里唯一
拿对比度换色相的一项——白底上「更亮」和「更高对比」是反向的，要在轨道色上稳过 3:1，橙就得压深、压深又滑回
琥珀棕，所以浅色那档落在轨道上 2.8、页面底色上 3.6。蓝和红不付这个税，两边都过 3:1。
